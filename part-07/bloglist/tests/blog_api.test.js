...
var token = null;
const INVALID_TOKEN =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6InRlc3RlciIsImlkIjoiNjMxY2NkM2MxZmQ2YzZkMjRjOGMwYTFkIiwiaWF0IjoxNjYyOTAzNTY3fQ.9aeIYthsYKPwCZgxDsOafgWNvS96T2TXd3Bp8uqmo84";
...
describe("when a blog is updated", () => {
  test("the number of blogs stays the same", async () => {
    ...
    await api.put(`/api/blogs/${id}`).send(data.blog).expect(200);
...
describe("when authorization token is not provided", () => {
  test("adding a blog returns 401", async () => {
    await api.post("/api/blogs").send(data.blog).expect(401);
  });
